...
\begin{register}[H]
  \caption{\hexrange{0000}{1FFF} - RAM\_EN - MBC1 RAM enable register}
  {
    \ttfamily
    \begin{tabularx}{\textwidth}{|X|X|X|X|X|X|X|X|}
      ...
      \hline
      \cellcolor{LightGray} & \cellcolor{LightGray} & \cellcolor{LightGray} & \cellcolor{LightGray} & \multicolumn{4}{c|}{RAM\_EN<3:0>} \\
      \hline
...
can be enabled by writing to the \hexrange{0000}{1FFF} address range a value
with the bit pattern \bin{1010} in the lower nibble. Upper bits don't
...
\hexrange{A000}{BFFF} are ignored, and reads return \hex{FF}. Pan Docs
...
\begin{speculation}
  ...
  possible that the \bin{1010} bit pattern check is done at write time and
...
\begin{table}[H]
  ...
  \begin{tabular}{|l|c|c|c|}
    \hline
    & \multicolumn{2}{c|}{Bank number} & Address within bank \\
    \hline
...
all reads return \hex{FF} and writes have no effect.
...
mapped to the \hexrange{A000}{BFFF} area. In MODE 1 the BANK2 register value is
...
\bin{1}, RAM\_EN is \bin{1010} and we are now reading a byte from \hex{B123}.
The actual physical RAM address that will be read is going to be \hex{5123} and
...
number within the selected game. At reset BANK2 is \bin{00}, and the "game"
...
ROM accesses, including accesses to \hexrange{0000}{3FFF}, use the BANK2
register value.
