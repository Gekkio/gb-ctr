...
register. The full 16-bit address is obtained by setting the most significant
byte to \hex{FF} and the least significant byte to the value of C, so the
possible range is \hexrange{FF00}{FFFF}.
...
immediate data \texttt{n}. The full 16-bit address is obtained by setting the
most significant byte to \hex{FF} and the least significant byte to the value
of \texttt{n}, so the possible range is \hexrange{FF00}{FFFF}.
...
the 8-bit A register. The full 16-bit address is obtained by setting the most
significant byte to \hex{FF} and the least significant byte to the value of
\texttt{n}, so the possible range is \hexrange{FF00}{FFFF}.
