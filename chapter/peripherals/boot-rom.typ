...
The 1-bit BANK register controls memory mapping of the boot ROM. Once #bin("1") has been written to it to unmap the boot ROM, it can only be mapped again by resetting the system.

#reg-figure(
  caption: [#hex("FF50") - BANK - Boot ROM mapping register]
)[
  #reg-table(
  ...
  #grid(
    columns: (auto, 1fr),
    gutter: 1em,
    [*bit 7-1*], [*Unimplemented*: Ignored during writes; reads return #bit("1") in a normal system],
    [*bit 0*], [
      *BOOT_OFF*: Boot ROM mapping bit\
      #bin("1") = Boot ROM is unmapped and #hex-range("0000", "00FF") works normally.\
      #bin("0") = Boot ROM is mapped and intercepts accesses to #hex-range("0000", "00FF").

      #v(1em)

      BOOT_OFF can only transition from #bin("0") to #bin("1"), so once #bin("1") has been written, the boot ROM is permanently unmapped until the next system reset. Writing #bin("0") never has any effect: it neither unmaps the boot ROM nor prevents a later write of #bin("1") from doing so.
    ]
  )
]
