...
\begin{register}[H]
  ...
  \begin{description}[leftmargin=5em, style=nextline]
    \item[bit 0]
      ...
      Writing to this register requests an OAM DMA transfer, but the write has
      an effect only after a delay of one extra machine cycle.

      Reading this register returns the last written value. The stored value is
      not cleared on reset, so the initial value is unknown and should not be
      relied on. However, 1st and 2nd generation devices tend to start with
      \hex{FF} as the initial value, and later generations tend to start with
      \hex{00}, so these values could be assumed if necessary.
  \end{description}
\end{register}
